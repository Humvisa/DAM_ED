...
public class AnalizadorCiberseguridad {
    ...
    public static void main(String[] args) {
        ...
        HashSet<Log> logsUnicos = new HashSet<>(logsBrutos);
        ...
        for (Log actual : logsUnicos){
            int frecuencia = 0;
            ataquesPorIP.put(actual.ipOrigen, frecuencia);
            for (Log actual1 : logsBrutos){
                if (actual.ipOrigen == actual1.ipOrigen){
                    frecuencia++;
                }
                ataquesPorIP.put(actual.ipOrigen, frecuencia);
            }
        ...
        for (String actual : nivelRiesgoZona.values()){
            for (Log actual1 : logsUnicos)
            if (actual1.ubicacion.equals(actual)){
                reporteRiesgo.put(actual, new ArrayList<>(actual1.idLog));
            }
