package tema5.lists.electroAvila;
...
public class GestionInventario {
    public static void main(String[] args) {
        ...
        Set<Producto> productosUnicos = new HashSet<>(); // Completar
        for (Producto actual : entradaAlmacen){
            productosUnicos.add(actual);
        }
        // TODO: TAREA 2 - Volver a pasar a una List para poder ordenar
        List<Producto> listaLimpia = new ArrayList<>();// Completar
        for (Producto actual : productosUnicos){
            listaLimpia.add(actual);
        }
        // TODO: TAREA 3 - Filtrar y Transformar
        // Quedarse solo con los de precioBase > 300 y subirles el precio un 21% (IVA).
        List<Producto> lista = new ArrayList<>();
        for (Producto actual : listaLimpia){
            if (actual.getPrecioBase() > 300){
                actual.setPrecioBase(actual.getPrecioBase()* 1.21);
                lista.add(actual);
            }
        }
        ...
        System.out.println("detectado filtrado "+ lista.size() + " registros");
